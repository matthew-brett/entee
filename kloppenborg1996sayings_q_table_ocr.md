---
# YAML metadata
title: "Appendix B: Comparative Stratigraphy of Q"
author: "John S. Kloppenborg"
linkcolor: blue
urlcolor: blue
mainfont: Gentium Plus
monofont: Menlo
header-includes:
- \usepackage{fontspec}
- \usepackage{polyglossia}
- \setdefaultlanguage{english}
- \setotherlanguage{greek}
---

Source: John S. Kloppenborg, scans of pp. 60–66 (PDF pages 1–7 of
`private/kloppenborg1996sayings_q_table.pdf`). Table begins on book p. 62
/ PDF p. 3. Cell values for disputed readings were back-ported from the
hi-res adjudication in `kloppenborg/kloppenborg_table_revised.md`.

Shading and border markings in the printed key and table are omitted here;
only the sigla themselves are recorded. Blank cells mean no printed
classification in that column (distinct from `--`, “not in Q”).

## Sigla

| Siglum | Meaning |
| --- | --- |
| `--` | not in Q |
| `?` | in Q but attribution uncertain |
| (box outline) | sub collections |

*Scan note*: the scan does not include the box outlines.

## Authors (table columns 1–8)

Abbreviations in parentheses are the column headers used in the comparative table below.

### 1. Schulz (1972) (Schu)

- **Q¹** — The kerygma of the oldest Palestinian-Syrian Q community
- **Q²** — The kerygma of the younger Syrian Q-community

### 2. Polag (1977) (Pol)

- **H** — Main Collection (*Hauptsammlung*)
- **Hᵃ** — Older clusters (*Die älteren Spruchgruppen*) predating the Main Collection
- **Hʳ** — Redactional insertions into the *Hauptsammlung*
- **SpR** — Late editing (*Spätredaktion*)

### 3. Jacobson (1978, 1992) (Jac)

- **Qᶜ** — compositional stage
- `Qᶜ*` — saying with redactional touches
- **Qᶜʳ** — redaction at the compositional stage
- **Qⁱ** — intermediate redaction
- **Q³** — final redaction

### 4. Schmithals (1979, 1980, 1985) (Schm)

- **Q¹** — nonkerygmatic, pre-Christian stratum
- `Q¹*` — pre-Christian stratum with slight Q² influence
- **Q²** — Christological (post-Markan) redaction of Q
- **Q²ʳ** — creation of the redactor of Q

### 5. Schenk (1981) (Sche)

- **Q** — included in Q
- **Qʳ** — from the final redactional stratum
- `Q*` — traditional but with slight redactional touches

### 6. Kloppenborg (1987, 1990) (Klo)

- **Q¹** — formative (instructional) stratum
- **Q²** — secondary, polemical stratum
- **Q²ʳ** — creation of the secondary redaction
- **Q³** — tertiary glosses

### 7. Sato (1988) (Sat)

- **A** — Redaction A [(A) = uncertain]
- **B** — Redaction B [(B) = uncertain]
- **C** — Redaction C [(C) = uncertain]
- **xʳ** — Redaction at the level of “A”, “B” or “C”
- **SpE** — *Spätere Einschübungen* (later interpolations—major redaction)
- **QLk** — Proto-Lukan recension of Q

### 8. Zeller (1977, 1982, 1984) (Zel)

- **Q¹** — Early Sayings-complexes
- **Q** — Other Q sayings & complexes
- **Q³** — Late Hellenistic-Jewish redaction of Q

## Comparative table

| Verse | Schu | Pol | Jac | Schm | Sche | Klo | Sat | Zel |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| (3:2-4) | `--` | `SpR?` |  | `Q²` | `--` | `Q²` | `A` | `Q` |
| 3:7-9 | `Q²` | `SpR` | `Qᶜ*` | `Q¹` | `Q` | `Q²` | `A` | `Q` |
| 3:16bd | `Q²` | `SpR` | `Qᶜ` | `Q²` | `Qʳ` | `Q²` | `A` | `Q` |
| 3:16c | `Q²` | `SpR` | `Qⁱ` | `Q²` | `Q` | `Q²` | `A` | `Q` |
| 3:17 | `Q²` | `SpR` | `Qᶜ` | `Q¹` | `Q` | `Q²` | `A` | `Q` |
| [3:21-22] | `--` | `SpR` | `Qᶜ` | `Q²` | `--` | `--` | `A` | `Q?` |
| 4:1-13 | `Q²` | `SpR` | `Q³` | `Q²` | `Qʳ` | `Q³` | `SpE` | `Q³` |
| [4:16] | `--` | `?` | `--` | `--` | `--` | `?` | `A` | `--` |
| <6:20a> | `--` | `H` | `?` | `?` | `Q` | `Q¹` | `A` | `Q` |
| 6:20b-21 | `Q¹` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `A` | `Q¹` |
| 6:22-23b | `Q²` | `H` | `Qᶜ` | `Q¹*` | `Qʳ` | `Q¹` | `A` | `Q¹` |
| 6:23c | `Q²` | `H` | `Qᶜʳ` | `Q¹` | `Qʳ` | `Q²ʳ` | `A` | `Q¹` |
| (6:24-26) | `--` | `?` | `--` | `Q¹?` | `--` | `Q²` | `QLk` | `QLk` |
| 6:27-28,32-33,35c | `Q¹` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `A` | `Q¹` |
| (6:34-35b) | `--` | `--` | `Qᶜ?` | `--` | `--` | `Q¹` | `A?` | `--` |
| 6:29-30 | `Q¹` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `A` | `Q¹` |
| (Q/Matt 5:41) | `--` | `H` | `--` | `--` | `--` | `Q¹` | `A` | `--` |
| 6:31 | `Q¹` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `A` | `Q¹` |
| 6:36 | `Q¹` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `A` | `Q¹` |
| 6:37b, 38c | `Q¹` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `A` | `Q¹` |
| (6:37c-38b) | `--` | `Hᵃ` | `Qᶜ?` | `--` | `--` | `Q¹` | `QLk` | `--` |
| 6:39b | `Q²` | `H` | `Qᶜʳ` | `Q?` | `Q` | `Q¹` | `SpE` | `?` |
| 6:40 | `Q²` | `H` | `Qᶜ` | `Q?` | `Q` | `Q¹` | `SpE` | `?` |
| 6:41-42 | `Q¹` | `H` | `Qᶜ*` | `Q?` | `Q` | `Q¹` | `A` | `Q¹` |
| 6:43-45 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `SpE` | `Q¹` |
| 6:46 | `Q²` | `H` | `Qᶜ` | `Q²?` | `Qʳ` | `Q¹` | `A` | `Q¹` |
| 6:47-49 | `Q²` | `H` | `Qᶜ` | `Q¹?` | `Q*` | `Q¹` | `A` | `Q¹` |
| 7:1a | `Q²` | `SpR` | `Qᶜʳ` | `Q²` | `Q` | `Q²` | `A` | `Q` |
| 7:1b-2, 6-10 | `Q²` | `SpR` | `Qᶜʳ` | `Q²` | `Q` | `Q²` | `A` | `Q` |
| 7:18-19, (20) | `Q²` | `Hᵃ` | `Qⁱ` | `Q²` | `Qʳ` | `Q²` | `A` | `Q` |
| 7:22 | `Q²` | `Hᵃ` | `Qⁱ` | `Q²` | `Q` | `Q²` | `A` | `Q` |
| 7:23 | `Q²` | `Hᵃ` | `Qⁱ` | `Q²` | `Qʳ` | `Q²` | `A` | `Q` |
| 7:24-26a | `Q²` | `Hᵃ` | `Qᶜ` | `Q²` | `Q*` | `Q²` | `A` | `Q` |
| 7:26b | `Q²` | `Hᵃ` | `Qᶜ` | `Q²` | `Qʳ` | `Q²` | `A` | `Q` |
| 7:27 | `Q²` | `SpR` | `Qᶜ` | `Q²` | `Qʳ` | `Q²` | `SpE` | `Q` |
| 7:28a | `Q²` | `SpR` | `Qⁱ` | `Q¹` | `Q` | `Q²` | `A` | `Q` |
| 7:28b | `Q²` | `SpR` | `Qⁱ` | `Q¹` | `Qʳ` | `Q²` | `A` | `Q` |
| [7:29-30] | `--` | `--` | `--` | `Q²` | `Qʳ` | `--` | `QLk` | `?` |
| 7:31-32 | `Q²` | `H` | `Qᶜʳ` | `Q¹` | `Q*` | `Q²` | `C` | `Q` |
| 7:33-35 | `Q²` | `H` | `Qᶜʳ` | `Q¹*` | `Qʳ` | `Q²` | `C` | `Q` |
| 9:57-58 | `Q²` | `H` | `Qᶜ` | `Q²` | `Q*` | `Q¹` | `B` | `Q` |
| 9:59-60 | `Q²` | `H` | `Qᶜ` | `Q²` | `Q` | `Q¹` | `B` | `Q` |
| (9:61-62) | `--` | `H` | `--` | `--` | `--` | `Q¹` | `QLk` | `--` |
| 10:2 | `Q²` | `H` | `Qⁱ` | `Q¹` | `Q` | `Q¹` | `Bʳ` | `Q¹` |
| 10:3 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `B` | `Q¹` |
| [Q/Matt 10:5-6] | `--` | `--` | `--` | `--` | `--` | `--` | `(B)` | `--` |
| [Q/Matt 10:23] | `--` | `H` | `--` | `Q¹` | `--` | `--` | `(B)` | `--` |
| 10:4-11 | `Q²` | `Hᵃ` | `Qᶜ*` | `Q¹` | `Q` | `Q¹` | `B` | `Q¹` |
| 10:12 | `Q²` | `H` | `Qᶜʳ` | `Q²ʳ` | `Qʳ` | `Q²ʳ` | `Cʳ` | `Q¹` |
| 10:13-15 | `Q²` | `H` | `Qᶜ` | `Q²` | `Q` | `Q²` | `C` | `Q` |
| 10:16 | `Q²` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q*` | `Q¹` | `Bʳ` | `Q¹` |
| 10:21 | `Q²` | `SpR` | `Qⁱ` | `Q¹` | `Q` | `Q²` | `B` | `Q` |
| 10:22 | `Q²` | `SpR` | `Qⁱ` | `Q²` | `Qʳ` | `Q²` | `SpE` | `Q³?` |
| 10:23b-24 | `Q²` | `SpR` | `?` | `Q²` | `Qʳ` | `Q²` | `B` | `Q` |
| [10:25-28] | `--` | `--` | `--` | `--` | `--` | `--` | `SpE?` | `Q?` |
| [11:1b] | `--` | `H` | `--` | `--` | `--` | `--` | `--` | `Q` |
| 11:2-4 | `Q¹` | `H` | `Qⁱ` | `Q¹` | `Q` | `Q¹` | `SpE` | `Q¹` |
| 11:9-10 | `Q¹` | `H` | `Qⁱ` | `Q?` | `Q` | `Q¹` | `SpE` | `Q¹` |
| 11:11-13 | `Q¹` | `H` | `Qⁱ` | `Q?` | `Q*` | `Q¹` | `SpE` | `Q¹` |
| 11:14-18a | `Q²` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q` | `Q²` | `C` | `Q` |
| 11:19 | `Q²` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q*` | `Q²` | `C` | `Q` |
| 11:20 | `Q²` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q` | `Q²` | `C` | `Q` |
| (11:21-22) | `--` | `Hᵃ` | `--` | `Q?` | `--` | `Q²` | `C` | `Q` |
| 11:23 | `Q²` | `Hᵃ` | `Qᶜ` | `Q¹` | `Q` | `Q²` | `C` | `Q` |
| 11:24-26 | `Q²` | `H` | `Qᶜ` | `Q?` | `Q` | `Q²` | `C` | `Q` |
| (11:27-28) | `--` | `--` | `Qᶜ` | `--` | `--` | `Q²` | `--` | `--` |
| 11:16, 29 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q*` | `Q²` | `C` | `Q` |
| 11:30 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Qʳ` | `Q²` | `C` | `Q` |
| 11:31-32 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q*` | `Q²` | `C` | `Q` |
| 11:33 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q²` | `SpE` | `Q` |
| 11:34-35 (36) | `Q²` | `H` | `Qᶜ` | `Q?` | `Qʳ` | `Q²` | `SpE` | `Q` |
| 11:39b-44, 46-48 | `Q¹` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q²` | `C` | `Q` |
| 11:42d | `Q¹` | `H` | `Qᶜ` | `Q¹` | `Qʳ` | `Q³` | `C` | `Q` |
| 11:49-51a | `Q²` | `Hʳ` | `Qᶜ` | `Q¹` | `Q*` | `Q²` | `C*` | `Q` |
| 11:51b | `Q²` | `Hʳ` | `Qᶜʳ?` | `Q¹` | `Qʳ` | `Q²ʳ` | `Cʳ` | `Q` |
| 11:52 | `Q¹` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q²` | `C` | `Q` |
| 12:2-3 | `Q²` | `H` | `Qⁱ?` | `Q¹` | `Q` | `Q¹` | `SpE` | `Q¹` |
| 12:4-7 | `Q¹` | `H` | `Qⁱ?` | `Q¹` | `Qʳ` | `Q¹` | `SpE` | `Q¹` |
| 12:8-9 | `Q¹` | `H` | `Qⁱ?` | `Q¹` | `Qʳ` | `Q²` | `SpE` | `Q¹` |
| 12:10 | `Q²` | `SpR` | `Qⁱ?` | `Q¹` | `Qʳ` | `Q²` | `SpE` | `Q¹` |
| 12:11-12 | `Q²` | `H` | `Q?` | `Q¹` | `Qʳ` | `Q¹` | `SpE` | `Q` |
| (12:13-14) | `--` | `--` | `--` | `--` | `--` | `Q¹` | `--` | `--` |
| (12:16-21) | `--` | `--` | `--` | `--` | `--` | `Q¹` | `QLk` | `--` |
| 12:22b-24, 26-28 | `Q¹` | `H` | `Qⁱ?` | `Q¹` | `Q*` | `Q¹` | `SpE?` | `Q¹` |
| 12:25 | `Q¹` | `H` | `Qⁱ?` | `Q¹` | `Qʳ` | `Q¹` | `SpE?` | `Q¹` |
| 12:29-31 | `Q¹` | `H` | `Qⁱ?` | `Q¹` | `Qʳ` | `Q¹` | `SpE?` | `Q¹` |
| 12:33-34 | `Q¹` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `SpE?` | `Q¹` |
| [12:35-38] | `--` | `H` | `Qᶜ` | `Q¹` | `--` | `--` | `QLk` | `Q¹?` |
| 12:39 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q*` | `Q²` | `SpE?` | `Q¹` |
| 12:40 | `Q²` | `H` | `Qⁱ?` | `Q¹` | `Qʳ` | `Q²` | `SpE?` | `Q¹` |
| 12:42b-46 | `Q²` | `H` | `Qⁱ?` | `Q¹` | `Q*` | `Q²` | `SpE?` | `Q¹` |
| [12:47-48] | `--` | `SpR` |  | `--` | `--` | `--` | `QLk` | `--` |
| (12:49) | `Q²` | `SpR` | `Qᶜ` | `--` | `--` | `Q²` | `SpE?` | `--` |
| [12:50] | `--` | `SpR` |  |  | `--` | `--` | `SpE?` | `--` |
| 12:51-53 | `Q²` | `SpR` | `Qᶜ` | `Q?` | `Q` | `Q²` | `SpE?` | `Q` |
| 12:54-56 | `--` | `H` | `Qᶜ` | `Q?` | `--` | `Q²` | `QLk` | `?` |
| 12:57-59 | `Q²` | `H` |  | `Q?` | `Q*` | `Q²` | `SpE?` | `?` |
| 13:18-19 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Qʳ` | `Q¹` | `SpE?` | `Q` |
| 13:20-21 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Qʳ` | `Q¹` | `SpE?` | `Q` |
| 13:23 | `--` |  |  |  | `Q` | `--` | `C` | `Q` |
| 13:24 | `Q²` | `H` |  | `Q¹` | `Q` | `Q¹` | `C` | `Q` |
| 13:(25), 26-27 | `Q²` | `H` |  | `Q¹*` | `Q` | `Q²` | `C` | `Q` |
| 13:28-29 | `Q²` | `H` |  | `Q²` | `Q` | `Q²` | `C` | `Q` |
| 13:30 | `--` | `H` |  | `Q¹` | `Q` | `Q²` | `--` | `--` |
| 13:34-35a | `Q²` | `H` | `Qᶜ` | `Q¹?` | `Qʳ` | `Q²` | `C` | `Q` |
| 13:35b | `Q²` | `H` | `Qᶜ` | `Q²ʳ` | `Qʳ` | `Q²ʳ` | `C` | `Q` |
| [14:5] | `--` | `H` |  | `--` | `Q` | `--` | `--` | `?` |
| 14:11 / 18:14 | `Q²` | `?` |  | `Q¹` | `Q` | `Q¹` | `--` | `?` |
| 14:16-24 | `Q²` | `H` | `Qᶜ` | `Q²` | `Q*` | `Q²` | `?` | `Q` |
| 14:26 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `?` | `Q` |
| 14:27 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q` | `Q¹` | `?` | `Q` |
| 17:33 | `Q²` | `H` | `Qᶜ` | `Q¹?` | `Q` | `Q¹` | `?` | `Q` |
| 14:34-35 | `Q²` | `H` | `Qᶜ` | `Q¹` | `Q*` | `Q¹` | `?` | `Q` |
| 15:4-7 | `Q²` | `H` |  | `Q¹?` | `Q*` | `Q¹` | `?` | `Q` |
| (15:8-10) | `--` | `H` | `?` | `--` | `--` | `Q¹` | `--` | `--` |
| 16:13 | `Q²` | `H` | `?` | `Q¹` | `Q` | `Q¹` | `?` | `Q` |
| 16:16 | `Q²` | `H` | `Qᶜʳ` | `Q¹` | `Qʳ` | `Q²?` | `?` | `Q` |
| 16:17 | `Q¹` | `H` | `?` | `Q¹` | `Q` | `Q³` | `?` | `Q` |
| 16:18 | `Q¹` | `H` | `?` | `Q¹` | `Q` | `Q¹` | `?` | `Q` |
| 17:1b | `--` | `H` | `?` | `Q¹` | `Q` | `Q¹` | `?` | `Q` |
| 17:2 | `--` | `H` | `?` | `Q¹` | `--` | `Q¹` | `--` | `--` |
| 17:3b-4 | `Q²` | `H` | `?` | `Q¹` | `Q` | `Q¹` | `?` | `Q` |
| 17:6b | `Q²` | `H` | `Qⁱ` | `Q¹` | `Q` | `Q¹` | `?` | `Q` |
| 17:23-24 | `Q²` | `H` |  | `Q¹` | `Q` | `Q²` | `C` | `Q¹` |
| 17:37b | `Q²` | `H` |  | `Q¹` | `Qʳ` | `Q²` | `C` | `Q¹` |
| 17:26-27, 30 | `Q²` | `H` |  | `Q¹` | `Q` | `Q²` | `C` | `Q¹` |
| (17:28-29) | `--` | `H` |  | `Q¹?` | `Q` | `Q²` | `QLk` | `Q?` |
| 17:34-35 | `Q²` | `H` |  | `Q¹` | `Q` | `Q²` | `C` | `Q¹` |
| 19:12-13, 15b-26 | `Q²` | `SpR` | `?` | `Q²` | `Q*` | `Q²` | `?` | `Q` |
| 22:28-30 | `Q²` | `H` | `?` | `Q¹*` | `Q*` | `Q²` | `?` | `Q` |

## Transcription notes

Automated OCR of the shaded/boxed table is unreliable for superscripts; this file was produced by combining Tesseract with careful visual reading of the scans, then corrected against the hi-res adjudication in `kloppenborg/kloppenborg_table_revised.md` (20 cells updated; see that file’s Adjudications section). Cells were checked against each author’s allowed sigla (e.g. Schenk only uses Q / Qʳ / Q*). Printed `?` attached to a siglum (e.g. `Q¹?`, `SpE?`) is retained as in the source. Asterisks on sigla (Q*, Q¹*, Qᶜ*, C*) use a normal `*` after any Unicode superscript letters—Unicode has no matching superscript asterisk glyph in the same set as ¹ ᵃ ʳ ᶜ. Jacobson’s Qᶜ* is especially easy to miss or to confuse with Qᶜʳ; 3:7-9 is confirmed Qᶜ* from the scan.

Most uncertain readings are small superscripts in Jacobson’s column (esp. Qⁱ vs Qᶜ on pp. 64–65) and a few Polag H / Hᵃ / Hʳ distinctions. Spot-check against the hi-res page images in `kloppenborg/kloppenborg_table_{1-5}.png` before citing individual cells.
